...
export function Timetable() {
  ...
  const filteredSchedule =
    selectedDay === 'All'
      ? schedule
      : schedule.filter(day => day.dayName === selectedDay)

  const dayNames = [
    'All',
    ...Array.from(new Set(schedule.map(day => day.dayName))),
  ]
  ...
  const getAvailabilityStatus = (session: ClassSession) => {
    ...
    if (available <= 2)
      return { text: `${available} spots left`, color: 'text-orange-600' }
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-sm text-muted-foreground'>
            ...
            <span
              className={
                dataSource === 'fitbox' ? 'text-primary' : 'text-orange-600'
              }
            >
      ...
      {error && (
        <Alert>
          ...
          <AlertDescription>
            {error}. Showing backup schedule from our content management system.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Important:</strong> All times shown in AEDT (Australian
          Eastern Daylight Time). Public holiday schedules may vary. Please call
          (03) 5234 5678 to confirm availability or book appointments.
        </AlertDescription>
      </Alert>
      ...
      {filteredSchedule.length === 0 && (
        <div className='text-center py-12'>
          ...
          <p className='text-muted-foreground'>
            Try selecting a different day or refresh the timetable.
          </p>
        </div>
      )}
